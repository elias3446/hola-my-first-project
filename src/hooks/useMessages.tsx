...
export const useMessages = (conversacionId: string | null) => {
  ...
  const calculateReadStatus = (message: Message, participantes: Participante[], isGroup: boolean): 'sent' | 'delivered' | 'read' => {
    ...
    // If no other participants (self-chat), check own ultimo_leido_at
    if (otherParticipants.length === 0) {
      const selfParticipant = participantes.find(p => p.user_id === message.user_id);
      if (!selfParticipant?.ultimo_leido_at) return 'sent';
      return new Date(selfParticipant.ultimo_leido_at) >= new Date(message.created_at) ? 'read' : 'sent';
    }
    ...
    if (isGroup && readByCount < otherParticipants.length) return 'delivered';
    return 'read';
  };
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      // Fetch participants with their read status
      ...
      // Fetch messages (excluding those hidden by current user)
