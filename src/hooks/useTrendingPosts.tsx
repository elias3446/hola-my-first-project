...
export const useTrendingPosts = (limit: number = 5, filterByCurrentUser: boolean = false) => {
  ...
  const fetchTrendingPosts = useCallback(async () => {
    try {
      ...
      if (filterByCurrentUser && profile?.id) {
  ...
  }, [limit, filterByCurrentUser, profile]);
  ...
  useEffect(() => {
    if (user && (!filterByCurrentUser || profile)) {
      fetchTrendingPosts();
    }
  }, [user, profile, fetchTrendingPosts, filterByCurrentUser]);
